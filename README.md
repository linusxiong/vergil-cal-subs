...
Turn your registered Columbia Vergil courses into a calendar subscription. Manually provide an **Access Token** and, optionally, a **Refresh Token** whenever you create or update a calendar. The service reads your courses and saves a snapshot; it **does not persist university tokens or refresh your schedule in the background**. Submit a fresh Access Token again to update the same subscription URL.
...
- One calendar per verified UNI, with an HTTPS / webcal subscription URL for Apple Calendar, Google Calendar, and Outlook; the URL stays stable unless you explicitly replace it.
...
## Get your Access Token (and optional Refresh Token)
...
5. Find `access_token` and copy the **complete, unmodified string** in its Value field. Optionally copy `refresh_token` from the same login session as well.
6. Paste the Access Token into its required field; the Refresh Token field can be left empty. Do not include quotes, a `Bearer ` prefix, or field names. Do not substitute `id_token` for the Access Token.
7. Check your term and submit. Both fields clear on every submission, including account lookup; paste a fresh Access Token and any optional Refresh Token again for another request, whether the previous request succeeded or failed.
...
4. Open **Response** and copy the JSON value of `access_token` and, optionally, `refresh_token` separately. Copy only each value, without JSON quotes or commas.
...
The university may rotate Refresh Tokens. Only if you supply a Refresh Token and an upstream request rejects the Access Token with HTTP 401, this project attempts at most one refresh during that same request and discards any returned tokens when the operation finishes. An expired Access Token without a Refresh Token returns HTTP 401; **obtain a fresh Access Token** from Vergil and submit again. Before your next update, obtain a fresh Access Token and, if wanted, a Refresh Token from the same session; sign in again if necessary. The project does not write rotated tokens back into Vergil's browser storage or guarantee that an old Refresh Token can be reused.
...
2. Optionally name your calendar. To exclude dates without classes, expand the options and enter one `YYYY-MM-DD` date per line, up to 366 dates. Exclusions apply to every course in the term being imported.
3. Paste your Access Token and, optionally, a Refresh Token, then create or update your calendar. Each verified UNI has one calendar: submitting again updates that existing calendar instead of creating a second one. On first creation, immediately save the **management link** and **subscription link** privately.
...
5. After adding or dropping courses, room changes, or changes to excluded dates, open your management link or use **Open existing calendar** on the homepage, provide a fresh Access Token and an optional Refresh Token, and update. Ordinary updates do not require adding the subscription URL again; explicitly replacing the URL does.
...
### Find your calendar and retain past semesters

1. On the homepage, paste a fresh **Access Token** (and an optional Refresh Token), then choose **Open existing calendar**. The server verifies your UNI with the university and matches its hash to your single calendar; typing a UNI alone cannot authorize access.
2. If a calendar exists, it opens automatically. Lookup returns zero or one normalized calendar snapshot with its imported term history, never school tokens or management keys. Lookup alone does not change your saved courses.
3. Both token fields clear after lookup. Paste a fresh Access Token and any optional Refresh Token again before updating. Select the term to import: its new snapshot replaces only that term, while previously imported semesters remain in the same ICS feed. The service does not automatically fetch all past university terms. Ordinary updates preserve the subscription URL and management link.

### Replace a subscription URL

If your subscription URL is exposed or you want a new one, replace it from the calendar page. You can authorize this with the private management key or, after account lookup, a fresh Access Token and optional Refresh Token. Confirm the warning before replacing it: **the old feed immediately returns 404, and you must subscribe again in your calendar app using the new URL**. Your saved courses, imported semesters, and management link remain unchanged. Replacement does not erase copies already cached by calendar clients.

Revocation is different: it deletes the calendar and still requires the saved private management key.

### The two types of links

| Link | Permissions | How to keep it |
| --- | --- | --- |
| `/calendar/<feed-ID>.ics` | Read the course calendar; anyone with the URL can view it | Share only with calendar clients that need your schedule |
| `/manage/<random-ID>#<management-key>` | View, update, replace the feed URL, and delete; updates also require a fresh Access Token for the original student | Save the complete link privately, including everything after `#` |
...
The original management key cannot be recovered because only its hash is stored. If you lose the management link, use **Open existing calendar** with a fresh Access Token to open your calendar instead; you can update it or replace its subscription URL without that key. Revocation still requires the original management key. If only the subscription URL leaks, replace it to keep your courses. Replacing the feed URL does not invalidate a leaked management key; if that key leaks and you still have it, revoke the calendar from its management page. Revocation deletes the snapshot from the active database and invalidates the old links, but cannot remove calendar-client caches or backups already held by service providers.
...
Open the local URL printed in the terminal, usually `http://127.0.0.1:5173`. Vite's Cloudflare plugin runs the Worker with local D1 state under `.wrangler/state`. Apply migrations before your first run. Local and production databases are separate. The migration enforces one calendar per owner and initializes existing feed IDs from calendar IDs, preserving existing subscription URLs. If an older instance has multiple calendars for one owner, the migration stops without deleting them; back up the database and reconcile those duplicates before retrying. Local development can use the all-zero database ID in the configuration; production must replace it.
...
```text
Browser: user submits Access Token + optional Refresh Token for this request only
    ↓ HTTPS POST
...
D1: term snapshots, combined ICS, calendar ID, feed ID, management-key hash, student hash, version
    ↑ GET /calendar/<feed-id>.ics (no university tokens required)
...
D1 stores normalized snapshots for imported terms, excluded dates, the last update time, the combined ICS, calendar and feed IDs, ETag, version, and hashes of the student identity and management key. The verified UNI is hashed to enforce one calendar per university account; no plaintext UNI is retained. The existing student identity hash restricts lookup, school-authorized updates, and feed URL replacement to the original student. It is a linkable pseudonymous identifier, **not a promise of anonymization**. Data does not expire automatically; it remains until the user revokes the calendar or the operator deletes it. Google, Apple, or Microsoft may also receive and cache course data when you subscribe.

The API contacts only fixed Columbia hosts and rejects cross-origin redirects and pagination across students. Each sync attempts at most one token refresh only after an upstream 401 and only when a Refresh Token was supplied; it neither broadens scopes nor performs registration writes. Updates use version-conditional writes to avoid concurrent overwrites. ICS responses support ETag / 304 and require cache revalidation.

## API and project structure

| Method | Path | Purpose |
| --- | --- | --- |
| POST | `/api/calendars` | Create or update the verified UNI’s single calendar; required Access Token, optional Refresh Token, term, optional name, and excluded dates; 201 for new, 200 for existing |
| GET | `/api/calendars/:id` | View a snapshot; management Bearer required |
| POST | `/api/calendars/lookup` | Find the account calendar; fresh Access Token and optional Refresh Token; verifies UNI ownership and returns `{calendars: [zero or one snapshot], refreshed}` |
| POST | `/api/calendars/:id/sync` | Replace only the chosen term snapshot; fresh Access Token and optional Refresh Token; verified UNI ownership also permits updates without management Bearer |
| POST | `/api/calendars/:id/rotate` | Replace only the feed URL; management Bearer with `{}` or fresh Access Token and optional Refresh Token; returns the calendar snapshot |
| DELETE | `/api/calendars/:id` | Revoke; management Bearer required |
| GET / HEAD | `/calendar/:feedId.ics` | Subscribe; the random URL grants read access |
...
- **Authentication failed:** sign in to Vergil again and copy a fresh Access Token; an optional Refresh Token must come from the same account and session. Do not use an ID Token. To use a private management link, keep its complete `#` key; alternatively, use verified account lookup on the homepage.
